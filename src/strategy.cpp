...
float Piece::Evaluate_nearest(Board* b, int color, int* cube_piece){
	...
	int red_num = b->red_piece_num;
	int blue_num = b->blue_piece_num;
	if( abs(red_num - blue_num) >= 2 ){
		final_score -= 1;
	}
	else{
		final_score += 1;
...
// 	for(int i=6;i<12;i++){
// 		if( b->cube_position[i] == -1 ) continue;
// 		if( b->cube_position[i]/5 <= blue_pos/5 && b->cube_position[i]%5 <= blue_pos%5 ){
// 			final_score += 1;
// 		}
// 	}
// 	int red_pos = cube_piece[1];
// 	for(int i=0;i<6;i++){
// 		if( b->cube_position[i] == -1 ) continue;
// 		if( b->cube_position[i]/5 >= red_pos/5 && b->cube_position[i]%5 >= red_pos%5 ){
// 			final_score += 1;
// 		}
// 	}
